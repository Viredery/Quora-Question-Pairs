...
def word_match_share(q1, q2, stops=None):
    ...
    R = divide((len(shared_words_in_q1) + len(shared_words_in_q2)) * 1.0, (len(q1words) + len(q2words)))
    return R
...
def q1_q2_wm_ratio(row):
    ...
    return divide(inter_wm, total_wm)
...
df_train['q1_q2_wm_intersect'] = df_train.apply(q1_q2_intersect, axis=1, raw=True)
df_test['q1_q2_wm_intersect'] = df_test.apply(q1_q2_intersect, axis=1, raw=True)

df_train[['q1_q2_wm_ratio', 'q1_q2_wm_intersect']].to_csv("{}/train_wm_feat.csv".format(config.feat_folder), index=False)
df_test[['q1_q2_wm_ratio', 'q1_q2_wm_intersect']].to_csv("{}/test_wm_feat.csv".format(config.feat_folder), index=False)